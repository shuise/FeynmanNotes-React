...
CRS.ua = function () {
  ...
  return UA.indexOf("msie") > -1
    ? UA.split("; ")[1].split(" ").join("/")
    : UA.indexOf("chrome") > -1
      ? ("chrome" + UA.split("chrome")[1]).split(" ")[0]
      : UA.indexOf("firefox") > -1
        ? "firefox" + UA.split("firefox")[1]
        : UA.indexOf("safari") > -1
          ? "safari" + UA.split("safari")[1]
          : UA.indexOf("opera") > -1
            ? UA.split(" ")[0]
            : UA;
};
...
CRS.jsonMerge = function (a, b, isWrite, filter) {
  for (var prop in b) if (isWrite || typeof a[prop] === "undefined" || a[prop] === null) a[prop] = filter ? filter(b[prop]) : b[prop];
  return a;
};
